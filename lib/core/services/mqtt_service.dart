...
class MqttService {
  ...
  static const String _MQTT_BASE = "devices/{{device_id}}" + _MQTT_BASE_DEVICE;
  ...
  Future<bool> connect({
    ...
    _espMessageStream.sink.add(EspMessage(
      espEventType: EspEventType.System,
      command: "Connecting to device [${this.deviceId}]",
      parameter: "",
    ));
  ...
  void _onConnected() {
    ...
    _mqttClient.subscribe(
      _parseTopicTemplate(_MQTT_GET_PORT1, deviceId),
      MqttQos.atMostOnce,
    );
    _mqttClient.subscribe(
      _parseTopicTemplate(_MQTT_GET_PORT2, deviceId),
      MqttQos.atMostOnce,
    );
    _mqttClient.subscribe(
      _parseTopicTemplate(_MQTT_GET_PING, deviceId),
      MqttQos.atMostOnce,
    );
    _mqttClient.subscribe(
      _parseTopicTemplate(_MQTT_GET_BEEPER, deviceId),
      MqttQos.atMostOnce,
    );
    _mqttClient.subscribe(
      _parseTopicTemplate(_MQTT_GET_SENSOR_DATA, deviceId),
      MqttQos.atMostOnce,
    );
    _mqttClient.subscribe(
      _parseTopicTemplate(_MQTT_LOG, deviceId),
      MqttQos.atMostOnce,
    );
    _mqttClient.subscribe(
      _parseTopicTemplate(_MQTT_UPTIME, deviceId),
      MqttQos.atMostOnce,
    );
  ...
  void sendCommand(EspEventType command, String parameter) {
    ...
    switch (command) {
      case EspEventType.OpenPort:
        payload = 'open';
        if (parameter == "1") {
          topic = _parseTopicTemplate(_MQTT_SET_PORT1, deviceId);
        } else if (parameter == "2") {
          topic = _parseTopicTemplate(_MQTT_SET_PORT2, deviceId);
        }
        break;
      ...
      case EspEventType.GetData:
        topic = _parseTopicTemplate(_MQTT_SET_SENSOR_DATA, deviceId);
        payload = 'data';
        break;
      case EspEventType.Ping:
        topic = _parseTopicTemplate(_MQTT_SET_PING, deviceId);
        payload = 'ping';
        break;
      case EspEventType.Beep:
        topic = _parseTopicTemplate(_MQTT_SET_BEEPER, deviceId);
        payload = 'beep';
        break;
      default:
    }
  ...
  // this method replaces the "IoT Device ID" from settings in the MQTT Topics
  // this was added to make it possible to connect to another device without
  // having to recompile the app
  String _parseTopicTemplate(String? topic, String? deviceId) {
    return topic!.replaceFirst("{{device_id}}", deviceId!);
  }
}
